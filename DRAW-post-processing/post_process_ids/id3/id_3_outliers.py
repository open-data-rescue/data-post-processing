...
def patch_outlier(entry):
    ...
    try:
        ...
        (float(value) < config.temperature_min or float(value) > config.temperature_max[str(entry[4])]) and entry[10] == 0:
                original_value=value
                value='{:.1f}'.format(float(original_value)/10.0)
                tables.add_error_edit_code(2, '321', original_value, value, entry)
                return value
