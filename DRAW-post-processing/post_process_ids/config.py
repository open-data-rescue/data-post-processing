...
# 9:time,
# 10: RH (%),
# 11: cloud cover (tenths),
# 12 :various (character)
...
# TODO : determine if any alterations necessary for non-pressure values
...
# values that can be disregarded automatically; method converts all data entries to lowercase to catch edge cases
# where term is capitalized or uncapitalized irregularly
...
# possible correct formats that a value can be in, for cases where we want to retrieve leading digits from it (True)
# or use the whole value (False):
...
temperature_min_max_fields = [[38, 36], [39, 37], [76, 78], [77, 79], [81, 76], [81, 77], [81, 38], [81, 39], [62, 76],
                              [62, 38], [62, 39]]

# TODO
# first field to compare to rest of the list - same observation time
temperature_less_than_other_fields = [[11, [9]], [12, [10]], [33, [9, 10]]]
# check that the first field is <= to past 24 hours of other field
temperature_min_fields = [[38, 9], [39, 10], [76, 9], [77, 10]]
# check that the first field is <= to past 24 hours of other field
temperature_max_fields = [[36, 9], [37, 10], [78, 9], [78, 10]]

temperature_air_wet_bulb_threshold = 15
# same observation time: abs(abs(field[0]-field[1])-abs(field[2]))<air_wet_bulb_threshold
temperature_air_wet_bulb = [[9, 11, 13], [10, 12, 13]]
...
# threshold value for which fluctuation between previous timestamp and current timestamp
# (for same field id) requires further investigation (phase 2)
# TODO : fill w/ pressure_fluctuation_stats results
scalar_fluctuation_thresholds = {'01': 0.30, '02': 0.40, '03': 0.50, '04': 0.50, '05': 0.50, '06': 0.50,
...
# intervals of interest for difference between transcribed and equation value -
# used for discerning values whose leading digits have (likely) been added incorrectly,
# hence the intervals occurring at +/- 1 range; configure the floats in the conditional statements
# according to your own statistical results (see stat_test_equations.py)
# TODO : fill in with same interval stats as other field id's below
def possible_wrong_lead_digs_id_4(difference_value):
    try:
