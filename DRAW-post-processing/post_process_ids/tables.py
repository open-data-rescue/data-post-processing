...
def update_fields_ppid(post_process_id, field_id_tuple):
    if type(field_id_tuple) is not tuple:
        field_id_tuple is '({})'.format(field_id_tuple)
    add_post_process_ids = "UPDATE fields " \
                           "SET post_process_id = {} " \
                           "WHERE id IN {};".format(post_process_id, field_id_tuple)
    print(add_post_process_ids)
    cursor.execute(add_post_process_ids)
    db_conn.commit()


#  command to create composite raw data table from data entries, fields and annotations tables;
#  creating this table is necessary as it enables the
#  addition of indexes (which speeds up code considerably during runtime) and s
#  tandardizes organization of data in DRAW post-processing
...
def create_duplicateless_table():
    ...
    cursor.execute("CREATE TABLE IF NOT EXISTS data_entries_corrected_duplicateless AS SELECT * "
                   "FROM data_entries_corrected LIMIT 0;")
...
def create_final_corrected_table(continue_flag):
    if continue_flag is False:
        ...
        create_table = ("CREATE TABLE data_entries_corrected_final AS SELECT * FROM "
                        "data_entries_corrected_duplicateless LIMIT 0;")
        cursor.execute(create_table)
...
def create_final_corrected_table_iso(continue_flag):
    if continue_flag is False:
        ...
        create_table = ("CREATE TABLE data_entries_corrected_final_iso AS SELECT * FROM "
                        " data_entries_corrected_final LIMIT 0;")
        cursor.execute(create_table)
...
def add_to_corrected_table(entry_id, value, user_id, page_id, field_id, field_key, annotation_id, transcription_id,
                           post_process_id, observation_date, flagged):
    global corrected_table
    corrected_table.append([entry_id, value, user_id, page_id, field_id, field_key, annotation_id, transcription_id,
                            post_process_id, observation_date, flagged])
...
def populate_corrected_table():
    sql_command = ("INSERT INTO data_entries_corrected "
                   "(id, value, user_id, page_id, field_id, field_key, annotation_id, transcription_id, "
                   "post_process_id, observation_date, flagged) "
...
# adds entry to "data_entries_corrected" table
def add_to_final_corrected_table_iso(entry_id, value, user_id, page_id, field_id, field_key, annotation_id,
                                     transcription_id, post_process_id, observation_date, flagged):
    global final_corrected_table_iso
    final_corrected_table_iso.append([entry_id, value, user_id, page_id, field_id, field_key, annotation_id,
                                      transcription_id, post_process_id, observation_date, flagged])
...
def add_to_final_corrected_table(entry_id, value, user_id, page_id, field_id, field_key, annotation_id,
                                 transcription_id, post_process_id, observation_date, flagged):
    global final_corrected_table
    final_corrected_table.append([entry_id, value, user_id, page_id, field_id, field_key, annotation_id,
                                  transcription_id, post_process_id, observation_date, flagged])
...
#  add flag or edit made to particular value to "data_entries_phase_{}_errors" table
#  (depending on chosen input parameter, can be for phase 1 or 2)
def add_error_edit_code(phase, error_code, original_value, corrected_value, entry_list, add_info=''):
    entry_id = entry_list[0]
    (user_id, page_id, field_id, field_key, annotation_id, transcription_id, post_process_id,
     observation_date) = [None for i in range(8)]
    if phase == 1:
        (user_id, page_id, field_id, field_key, annotation_id, transcription_id, post_process_id,
         observation_date) = entry_list[2:]  # deal with 10 rows
        phase_1_errors.append([entry_id, original_value, corrected_value, error_code, user_id, page_id,
                               field_id, field_key, annotation_id, transcription_id, post_process_id,
                               observation_date, add_info])
    elif phase == 2:
        (user_id, page_id, field_id, field_key, annotation_id, transcription_id,
         post_process_id, observation_date) = entry_list[2:len(entry_list) - 1]  # deal with 11 rows
        phase_2_errors.append([entry_id, original_value, corrected_value, error_code, user_id, page_id,
                               field_id, field_key, annotation_id, transcription_id, post_process_id,
                               observation_date, add_info])
...
def populate_error_edit_code(phase):
    sql_command = ("INSERT INTO data_entries_phase_{}_errors "
                   "(id, ORIGINAL_VALUE, CORRECTED_VALUE, error_code, user_id, page_id, field_id, "
                   "field_key, annotation_id, transcription_id, post_process_id, observation_date, additional_info) "
    ...
    #  data frame.to_sql
        df_temp = pd.DataFrame(phase_2_errors,
                               columns=['id', 'ORIGINAL_VALUE', 'CORRECTED_VALUE', 'error_code', 'user_id', 'page_id',
                                        'field_id', 'field_key', 'annotation_id', 'transcription_id', 'post_process_id',
                                        'observation_date', 'additional_info'])
...
def add_to_duplicateless_table(entry_id, value, user_id, page_id, field_id, field_key, annotation_id, transcription_id,
                               post_process_id, observation_date, flagged):
    global duplicateless
    duplicateless.append([entry_id, value, user_id, page_id, field_id, field_key, annotation_id, transcription_id,
                          post_process_id, observation_date, flagged])
...
def populate_duplicateless_table():
    sql_command = ("INSERT INTO data_entries_corrected_duplicateless "
                   "(id, value, user_id, page_id, field_id, field_key, "
                   "annotation_id, transcription_id, post_process_id, observation_date, flagged) "
                   "VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);")
    cursor.executemany(sql_command, duplicateless)
    db_conn.commit()


# u pdates duplicateless table - used to update MySQL table during observation reconciliation,
#  before continuing with phase 2
def update_duplicateless_table(value, entry_id):
    sql_command = "UPDATE data_entries_corrected_duplicateless SET value = %s WHERE id = %s;"
