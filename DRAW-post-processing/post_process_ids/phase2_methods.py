# File in which all methods that are called upon in pressure statistical workflow are stored,
...
# todo humidity equations
def check_other_transcription_errors():
    pass   # TODO : find possible transcription errors e.g. digits in wrong order


# returns True or False, based on whether the function determines that the fluctuation between
# the current timestamp and BOTH adjacent timestamps
                ...
                if time_delta_2 < config.time_delta_limit:
                    try:
                        if config.possible_pressure_formats(adjacent_day_entries[index][1], False):
                            scalar_value = (abs(float(entry_list[1]) - float(adjacent_day_entries[index][1])) /
                                            time_delta_2)
...
# returns the resultant value for field_id based on equation 1 or 2 (as indicated by equation_num),
# given presence of associated variables
        ...
        case 1:
            ...
            match field_ids_sorted:
                ...
                case _:
                    return None
            try:
                baro_32 = (round(float(baro_inst_cor) * (1 - ((0.000101*(float(atb) - 32) - 0.0000102*(float(atb) - 62))
                                                              / (1 + 0.000101*(float(atb) - 32)))), 3))
                return baro_32
            except ValueError:
                return None
        ...
        case 3:
            ...
            match field_ids_sorted:
                ...
                case _:
                    return None
            try:
            #  (1 - ((0.000101 * (float(atb) - 32) - 0.0000102 * (float(atb) - 62)) /
            #  (1 + 0.000101 * (float(atb) - 32))))
                baro_inst_cor = (round(float(baro_32) / (1 - ((0.000101 * (float(atb) - 32) - 0.0000102 *
                                 (float(atb) - 62)) / (1 + 0.000101 * (float(atb) - 32)))), 3))
