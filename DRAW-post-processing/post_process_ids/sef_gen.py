...
def correctObservationDate(sef_type, observation_date):
    ...
    elif config.sef_apply_utc is True:
...
def generateSEF(sef_type):
    ...
    if type(config.sef_type_to_field_id[sef_type]) is int:
        query = ("select value,observation_date from data_entries_corrected_final_iso where field_id = {} "
                 "order by observation_date asc".format(config.sef_type_to_field_id[sef_type]))
    else:
        query = ("select value,observation_date from data_entries_corrected_final_iso where field_id in {} order by "
                 "observation_date asc".format(config.sef_type_to_field_id[sef_type]))
    ...
    for result in results:
        ...
        try:
            result_str = (str(observation_date.year) + "\t" + str(observation_date.month) + "\t" +
                          str(observation_date.day) + "\t" + str(observation_date.hour) + "\t" +
                          str(observation_date.minute + "\t" + getPeriod(sef_type) + "\t"+value + "\t|\t\n"))
            type_result_set.append(result_str)
        finally:
            print("Couldn't generate SEF line for value=" + str(value) + ", observation date =" +
    ...
    if filename is not None:
        ...
        f.write("Meta\t")
        f.write("UTCOffset=")
        if (config.sef_apply_utc is True):
